...
io.on('connection', (socket) => {
  ...
  socket.on('send_garbage', ({ lines }: { lines: number }) => {
    const room = findRoom(socket.id);
    if (!room || room.status !== 'playing') return;
    const count = Math.min(Math.max(1, Math.floor(lines)), 8);
    socket.to(room.id).emit('receive_garbage', { lines: count, fromId: socket.id });
  });
